...
#> simplenergy:custom_blocks/servo_extractor/place_main
#
# @executed	as @e[type=item_frame,tag=...] & at @s
#
# @within	simplenergy:custom_blocks/servo_extractor/search [ as @e[type=item_frame,tag=...] & at @s ]
#

# Get the facing direction of the item frame
scoreboard players set #item_frame_facing simplenergy.data 1
execute if entity @s[type=item_frame] run function simplenergy:custom_blocks/servo_extractor/get_facing
...
execute as @n[tag=simplenergy.new] at @s run function simplenergy:custom_blocks/servo_extractor/place_secondary
...
scoreboard players add #total_servo_extractor simplenergy.data 1
