...
# Decode errors
execute if score #mcload_error simplenergy.data matches 1 run tellraw @a {"text":"SimplEnergy Error: This version is made for Minecraft 1.21+.","color":"red"}
execute if score #dependency_error simplenergy.data matches 1 run tellraw @a {"text":"SimplEnergy Error: Libraries are missing\nplease download the right SimplEnergy datapack\nor download each of these libraries one by one:","color":"red"}
execute if score #dependency_error simplenergy.data matches 1 unless score #common_signals.major load.status matches 0.. unless score #common_signals.patch load.status matches 2.. run tellraw @a {"text":"- [Common Signals]","color":"gold","clickEvent":{"action":"open_url","value":"https://github.com/Stoupy51/CommonSignals"}}
...
execute if score #dependency_error simplenergy.data matches 1 unless score #energy.major load.status matches 1.. unless score #energy.minor load.status matches 8.. run tellraw @a {"text":"- [DatapackEnergy]","color":"gold","clickEvent":{"action":"open_url","value":"https://github.com/ICY105/DatapackEnergy"}}
