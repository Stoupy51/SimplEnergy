...
"""
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/tick", content)

	# Gui for each slot
	slots: str = "\n".join(f"function {ns}:custom_blocks/pulverizer/gui_passive_slot {{\"index\":{i},\"slot\":{i+9}}}" for i in range(PULVERIZER_SLOTS))
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/gui_for_each_slot", f"""
...
""")
	...
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/gui_passive_slot", f"""
...
""")
	...
	for i, progression in enumerate(progressions_cmd):
		if i == 0:
			progressions_gui.append(f"$execute if score #progression {ns}.data matches ..0 run item replace block ~ ~ ~ container.$(slot) with {GUI_VANILLA_ITEM}[item_model=\"{gui[progression]}\",{GUI_DATA}]")
		elif i == (nb_gui - 1):
			progressions_gui.append(f"$execute if score #progression {ns}.data matches {previous_max + 1}.. run item replace block ~ ~ ~ container.$(slot) with {GUI_VANILLA_ITEM}[item_model=\"{gui[progression]}\",{GUI_DATA}]")
		else:
			gui_min: int = previous_max + 1
	...
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/gui_progression", "\n".join(progressions_gui))


	# Handle intruder item on gui
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/handle_item_on_gui", f"""
...
""")
	...
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/work", f"""
...
""")
	...
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/reset_progress", f"""
scoreboard players set #progression {ns}.data 0
...
""")
	...
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/gui_active_slot", f"""
# Get progression
scoreboard players set #progression {ns}.data 0
...
""")
	...
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/get_pulverizer_recipe", f"""
# Get the recipe
...
""")
	...
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/get_max_stack_size", f"""
# Copy item
...
{content}
""")
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/entity_get_max_stack_size", f"""
...
""")
	...
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/destroy", f"""# Copy slots to storage
data remove storage {ns}:temp slots
data modify storage {ns}:temp slots set from entity @s item.components."minecraft:custom_data".{ns}.pulverizer_slots
\n""", prepend = True)
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/replace_item", f"""
...
""")
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/place_secondary", f"""
...
"""
	...
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/place_secondary", content)
	write_to_function(config, f"{ns}:custom_blocks/pulverizer/destroy", "# ItemIO compatibility\nfunction #itemio:calls/container/destroy\n\n", prepend = True)
