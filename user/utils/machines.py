...
"""
	...
	# Electric Smelter
	energy = database["electric_smelter"]["custom_data"]["energy"]
	nb_gui = sum("electric_smelter_" in x for x in gui) - 2	# -2 because we don't count the 0 and 5
	gui_0 = gui["gui/electric_smelter_0.png"]
	gui_1, min_1, max_1 =	gui["gui/electric_smelter_1.png"],	1, 			1 * energy["max_storage"] // nb_gui - 1
	gui_2, min_2, max_2 =	gui["gui/electric_smelter_2.png"],	max_1 + 1,	2 * energy["max_storage"] // nb_gui - 1
	gui_3, min_3, max_3 =	gui["gui/electric_smelter_3.png"],	max_2 + 1,	3 * energy["max_storage"] // nb_gui - 1
	gui_4, min_4, max_4 =	gui["gui/electric_smelter_4.png"],	max_3 + 1,	4 * energy["max_storage"] // nb_gui - 1
	gui_5, min_5 =			gui["gui/electric_smelter_5.png"],	max_4 + 1
	default_cmd = database["electric_smelter"]["custom_model_data"]
	working_cmd = default_cmd + 1
	content = f"""
...
execute if score @s energy.storage matches {energy["usage"]}.. if data storage {namespace}:temp all.Items[{{Slot:0b}}] run function {namespace}:custom_blocks/electric_smelter/work
...
execute if score #cook_time {namespace}.data matches 1.. if score #second {namespace}.data matches 0 run playsound {namespace}:electric_smelter block @a[distance=..12] ~ ~ ~ 0.25
"""
	write_to_file(f"{custom_blocks}/electric_smelter/tick.mcfunction", content)
	content = f"""
...
execute if score #cook_time {namespace}.data matches 1.. run scoreboard players add #cook_time {namespace}.data 7
...
"""
	write_to_file(f"{custom_blocks}/electric_smelter/work.mcfunction", content)
