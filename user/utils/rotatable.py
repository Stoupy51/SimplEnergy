...
def setup_rotatable_tags(config: dict, rotatables: list[str]) -> None:
	...
	write_to_tags(config, "mechanization:function/wrench_break", stp.super_json_dump(json_content))
	json_content = {"required": False, "values": [f"{namespace}:calls/mechanization/wrench_modify"]}
	write_to_tags(config, "mechanization:function/wrench_modify", stp.super_json_dump(json_content))

	# Write slots functions
	write_to_function(config, f"{namespace}:calls/mechanization/wrench_break", f"""
