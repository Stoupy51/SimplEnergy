...
execute store result storage simplenergy:energy Energy int 1 run scoreboard players get @s EF_Joule
...
data modify entity @e[type=minecraft:item,limit=1,sort=nearest,distance=..1] Item.tag.BlockEntityTag.Items[0].tag.Energy set from storage simplenergy:energy Energy
setblock ~ ~ ~ oak_sign{Text1:'[{"text":"[Charge : ","italic":false,"color":"gray"},{"nbt":"Part1","storage":"simplenergy:energy"},{"text":"."},{"nbt":"Part2","storage":"simplenergy:energy"},{"text":" kJ]"}]'}
