#Kill undesired Items & Replace the item to keep Motion
    ...
    data modify entity @e[type=minecraft:item,nbt={Item:{id:"minecraft:barrel"}},limit=1,sort=nearest,distance=..1] Item set from storage simplenergy:items 2012102
...
#Update the Battery to keep Energy
    ...
    execute if score @s EF_Joule matches 1.. run data modify entity @e[type=minecraft:item,limit=1,sort=nearest,distance=..1,nbt={Item:{tag:{SE_EliteBattery:1b}}}] Item.tag.BlockEntityTag.Items[0].tag.Energy set from storage simplenergy:energy X
