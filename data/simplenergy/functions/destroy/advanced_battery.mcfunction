#Kill undesired items and replace the item
    ...
    data modify entity @e[type=minecraft:item,nbt={Item:{id:"minecraft:barrel"}},limit=1,sort=nearest,distance=..1] Item set from storage simplenergy:items 2012101
...
#Update the Battery to keep Energy
    ...
    execute if score @s EF_Joule matches 1.. run data modify entity @e[type=minecraft:item,limit=1,sort=nearest,distance=..1,nbt={Item:{tag:{SE_AdvancedBattery:1b}}}] Item.tag.BlockEntityTag.Items[0].tag.Energy set from storage simplenergy:energy X
    ...
    execute if score @s EF_Joule matches 1.. run data modify entity @e[type=minecraft:item,limit=1,sort=nearest,distance=..1,nbt={Item:{tag:{SE_AdvancedBattery:1b}}}] Item.tag.display.Lore append from block ~ ~ ~ Text1
