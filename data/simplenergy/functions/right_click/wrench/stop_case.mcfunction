...
execute store result score #success simplenergy.data as @e[type=glow_item_frame,tag=simplenergy.destroyer,tag=!simplenergy.cable,limit=1,sort=nearest,dx=0,dy=0,dz=0] at @s if block ~ ~ ~ furnace run function simplenergy:right_click/wrench/rotate
