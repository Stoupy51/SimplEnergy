...
#Timer & Other
	...
	execute if score #second_5 simplenergy.data matches 100.. run function simplenergy:tick_second_5
	execute as @e[type=glow_item_frame,tag=simplenergy.tick_glows,sort=random] at @s run function simplenergy:utils/tick_glows
	execute as @e[type=item,tag=!simplenergy.checked] at @s run function simplenergy:destroy/item_check/main
