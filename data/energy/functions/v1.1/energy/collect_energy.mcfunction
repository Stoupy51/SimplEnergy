...
scoreboard players operation #energy.cable_rate energy.data = @e[type=#energy:valid_block_entities,tag=energy.cable,predicate=energy:v1.1/equals_network_id,limit=1] energy.transfer_capacity

#collect power
scoreboard players set #energy.available energy.data 0
execute as @e[type=#energy:valid_block_entities,tag=energy.send,predicate=energy:v1.1/has_network_id,scores={energy.storage=1..}] run function energy:v1.1/energy/collect_energy_2
