...
execute if score @s energy.transfer_rate < @s energy.transfer_capacity run scoreboard players operation @e[type=#energy:valid_block_entities,tag=energy.cable,predicate=energy:v1.4/equals_network_id] energy.transfer_capacity = @s energy.transfer_rate
