...
scoreboard players set $temp2 smd.data 1000
execute store result score @s smd.data run data get block ~ ~ ~ Items[{Slot:2b}].Count
execute if score @s smd.data matches 1.. if score @s smd.data < $temp2 smd.data run scoreboard players operation $temp2 smd.data = @s smd.data
...
execute if score $temp2 smd.data matches 1..64 run function smithed:crafter/impl/v0_0_1/block/table/crafting/output/shift_click/calculate_extras
